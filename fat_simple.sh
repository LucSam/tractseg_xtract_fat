...
# Atlas data derive from the WU-Minn Human Connectome Project; please acknowledge HCP.
# Standalone FAT workflow: share this file alone; the Python helper is embedded.
# Requires MRtrix3, ANTs, FSL Harvard-Oxford atlas, curl, and Python 3 with NumPy/NiBabel/SciPy.
...
run_fat_pipeline() {
  ...
  : "${FSLDIR:?Set FSLDIR to the FSL installation containing the Harvard-Oxford atlas.}"
  export MPLCONFIGDIR="$OUT/work/matplotlib"
  export XDG_CACHE_HOME="$OUT/work/cache"

  mkdir -p "$OUT/work" "$OUT/bundle_segmentations" "$OUT/bundle_segmentations_original" "$OUT/endings_segmentations" "$OUT/iFOD2_trackings" "$OUT/SD_STREAM_trackings" "$MPLCONFIGDIR" "$XDG_CACHE_HOME"

  # HCP1065 bundle prior and broad cortical targets, each in its correct template space.
  # The helper downloads/checks the two atlas archives once; no pretrained weights needed.
  fat_qc inputs --fod "$IN/wm.nii.gz"
  atlas_args=(--threads 4 --atlas-dir "${ATLAS_DIR:-$PWD/atlases/hcp1065}")
  ...
  fat_qc atlas-to-subject --t1 "${T1:-$IN/t1_brain.nii.gz}" --reference "$IN/wm.nii.gz" --fsl-dir "$FSLDIR" --out "$OUT/work/atlas" "${atlas_args[@]}"

  # Threshold in the subject's diffusion grid; retain a connected mask with a bounded 3 mm registration margin.
  fat_qc probability-bundle "$OUT/work/atlas/FAT_left_probability.nii.gz" "$OUT/bundle_segmentations_original/FAT_left.nii.gz" "$OUT/bundle_segmentations/FAT_left.nii.gz"
  fat_qc probability-bundle "$OUT/work/atlas/FAT_right_probability.nii.gz" "$OUT/bundle_segmentations_original/FAT_right.nii.gz" "$OUT/bundle_segmentations/FAT_right.nii.gz"

  # IFG pars opercularis/triangularis -> superior frontal gyrus plus SMA; 3 mm ROI margin.
  fat_qc endings "$OUT/work/atlas/endings_native.nii.gz" "$OUT/bundle_segmentations/FAT_left.nii.gz" left "$OUT"
  fat_qc endings "$OUT/work/atlas/endings_native.nii.gz" "$OUT/bundle_segmentations/FAT_right.nii.gz" right "$OUT"

  # Grow in both directions without early ROI stopping; validate actual endpoints below.
  ...
  tckgen "$IN/wm.nii.gz" "$OUT/work/FAT_left_iFOD2.tck" -algorithm iFOD2 -seed_image "$OUT/seed_masks/FAT_left_b.nii.gz" -include "$OUT/endings_segmentations/FAT_left_b.nii.gz" -include "$OUT/endings_segmentations/FAT_left_e.nii.gz" -mask "$OUT/bundle_segmentations/FAT_left.nii.gz" -select 4000 -seeds 2000000 -minlength 20 -maxlength 150 -cutoff 0.05 -downsample 1 -nthreads 4
  ...
  tckgen "$IN/wm.nii.gz" "$OUT/work/FAT_right_iFOD2.tck" -algorithm iFOD2 -seed_image "$OUT/seed_masks/FAT_right_b.nii.gz" -include "$OUT/endings_segmentations/FAT_right_b.nii.gz" -include "$OUT/endings_segmentations/FAT_right_e.nii.gz" -mask "$OUT/bundle_segmentations/FAT_right.nii.gz" -select 4000 -seeds 2000000 -minlength 20 -maxlength 150 -cutoff 0.05 -downsample 1 -nthreads 4
  ...
  tckgen "$IN/wm.nii.gz" "$OUT/work/FAT_left_SD_STREAM.tck" -algorithm SD_STREAM -seed_image "$OUT/seed_masks/FAT_left_b.nii.gz" -include "$OUT/endings_segmentations/FAT_left_b.nii.gz" -include "$OUT/endings_segmentations/FAT_left_e.nii.gz" -mask "$OUT/bundle_segmentations/FAT_left.nii.gz" -select 4000 -seeds 2000000 -minlength 20 -maxlength 150 -cutoff 0.05 -downsample 1 -nthreads 4
